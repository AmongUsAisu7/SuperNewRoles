...
public class GetSampleAbility : TargetCustomButtonBase
{
    ...
    public LoversCouple TakeSample { get; private set; }

    public GetSampleAbility(float coolTime, string buttonText, Sprite sprite, bool IsLoversMe, Action<List<ExPlayerControl>> callback = null, bool enabledTimeLimit = false, float timeLimit = 0f)
    {
        CoolTime = coolTime;
    ...
    private void OnFixedUpdate()
    {
        ...
        if (ModHelpers.Not(!_created)) return;
    ...
    public override void OnClick()
    {
        ...
        {
            RpcCustomGetSample(CurrentTarget, Target);
            _callback?.Invoke([CurrentTarget, Target]);
        }
    }
    [CustomRPC]
    public void RpcCustomGetSample(ExPlayerControl playerA, ExPlayerControl playerB)
    {
        //TakeSample = AssignRoles.CustomSetSample(playerA, playerB, AssignRoles.LoversIndex, true);
        // キューピッド能力にも作成したラバーズの情報を同期しておく
        if (Player.TryGetAbility<ClinicalLaboratoryTechnicianAbility>(out var clinicalLaboratoryTechnicianAbility))
        {
            clinicalLaboratoryTechnicianAbility.SetSample(playerA.PlayerId, playerB.PlayerId);
        }
        _created = true;
